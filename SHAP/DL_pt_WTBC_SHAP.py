...
# with open('./'+'study_'+str(modtype)+str(ID)+str(wavelet)+'.pkl','rb') as f:
...
for init in range(initm):
    ...
    globals()["model"+str(init)]=tf.keras.models.load_model(r'/home/chidesiv/Desktop/Scripts_py/revised_version_scripts/PTWSEQ/20pct_PT/'+'best_model'+str(modtype)+str(init)+str(ID)+str(wavelet)+'.h5')
...
# pyplot.savefig('./'+'Well_ID'+str(ID)+str(modtype)+str(initm)+str(wavelet)+'PT.png', dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
...
shap.summary_plot(s, X,feature_names =  ['P1','P2','P3','P4','P5','T1','T2','T3','T4','T5'], show=False,color_bar=False)              
plt.colorbar()
