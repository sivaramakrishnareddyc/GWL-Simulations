...
def Wavelet_transform(X_train_s):
    ...
    li=[np.transpose(X_wav_tr_c1) ,X_sc_tr_c1]
...
import pickle
# pickle.dump(Study_DL,open('./'+'study_'+str(modtype)+str(ID)+str(wavelet)+'PE.pkl', 'wb'))

with open('/home/chidesiv/Desktop/Scripts_py/revised_version_scripts/PE/20pct_WTPE_lev4/'+'study_'+str(modtype)+str(ID)+str(wavelet)+'PE.pkl','rb') as f:
...
Middle_Target_Index = y_train.max()*0.93 #- y_train.min()/2
# #text
...
pyplot.legend(ncol=3,fontsize=36,loc='best')
pyplot.title(str(modtype)+' '+ "PE"+' - '+" WT:"+str(wavelet)+ ' Test set RMSE :' + str("%.2f" % scores_va.RMSE_va[0]), size=24,fontweight = 'bold')

# pyplot.title(str(modtype)+" Test PE:"+str(ID)+" WT:"+str(wavelet), size=17,fontweight = 'bold')
...
pyplot.savefig('./'+'Well_ID'+str(ID)+str(modtype)+str(initm)+str(wavelet)+'PE_Test_f.png', dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
...
# pd.DataFrame(mae_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PEmae.csv')    
# pd.DataFrame(mse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PEmse.csv')   
# pd.DataFrame(rmse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PErmse.csv')   
# pd.DataFrame(r2_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PEr2.csv') 
