...
from wmtsa.modwt import modwt,cir_shift,choose_nlevels
...
data =pd.read_csv(r"/home/chidesiv/Desktop/Scripts_py/selectedstations/data/PE/Well_ID"+str(ID)+"data_PT76.csv")

data['date_p'] = pd.to_datetime(data['date_p'])

X_train, X_test, y_train, y_test, indices_train, indices_test = train_test_split(  data[['prec','temp']], data["val_calc_ngf"], data['date_p'],test_size=test_size, random_state=1, shuffle=False)
...
def Wavelet_transform(X_train_s):
    
    X_wav_tr1,X_sc_tr1=modwt(X_train_s[:,0],wtf=wavelet,nlevels=4,boundary='periodic',RetainVJ=False)
    # choose_nlevels(choice='conservative', wtfname=wavelet, N=len(X_train_s))
     
    X_wav_tr_c1,X_sc_tr_c1=cir_shift(X_wav_tr1,X_sc_tr1) 
 
    X_wav_tr2,X_sc_tr2=modwt(X_train_s[:,1],wtf=wavelet,nlevels=4,boundary='periodic',RetainVJ=False)
    
   # choose_nlevels(choice='conservative', wtfname=wavelet, N=len(X_train_s))
    X_wav_tr_c2,X_sc_tr_c2=cir_shift(X_wav_tr2,X_sc_tr2) 

 
    li=[np.transpose(X_wav_tr_c1) ,X_sc_tr_c1,np.transpose(X_wav_tr_c2) ,X_sc_tr_c2]
...
def Wavelet_transform1(X_train_s):
   
    
    
    X_wav_tr1,X_sc_tr1=modwt(X_train_s[:,0],wtf=wavelet,nlevels=4,boundary='periodic',RetainVJ=False)
    
    # choose_nlevels(choice='conservative', wtfname=wavelet, N=len(X_train_s)/2)
    X_wav_tr_c1,X_sc_tr_c1=cir_shift(X_wav_tr1,X_sc_tr1) 
...
#removing boundary affected coefficients 
#J_0 < log2(240/(20-1) +1)
#L=(2^J0 – 1)*(L-1)+1 
...
X_C_w1=Wavelet_transform1(X_C_scaled)



#Removing boundary affected coefficients in the beginning
# lev=choose_nlevels(choice='conservative', wtfname=wavelet, N=len(X_train_s))
# lev1=choose_nlevels(choice='conservative', wtfname=wavelet, N=len(X_C_scaled)*(1-test_size))
lev=4
lev1=4

f=((int(pow(2, lev))-1)*(L-1))+1
f1=((int(pow(2, lev1))-1)*(L-1))+1

X_c, y_c = reshape_data(np.array(X_C_w1),y_C_scaled ,seq_length=seq_length)
...
# X_valid_l=X_c[int((len(X_c)+seq_length+f-1)*(1-test_size)-seq_length-f+1):]
# y_valid_l=y_c[int((len(X_c)+seq_length+f-1)*(1-test_size)-seq_length-f+1):]
X_valid_l=X_C_w[int((len(X_train_l))):]
y_valid_l=y_c_w[int((len(y_train_l))):]
...
Study_DL= optuna.create_study(direction='minimize',sampler=TPESampler(seed=10),study_name='study_'+str(modtype)+str(wavelet)+str(ID))
...
import pickle
# pickle.dump(Study_DL,open('./'+'study_'+str(modtype)+str(ID)+str(wavelet)+'PT.pkl', 'wb'))



# pickle.dump(Study_DL,open('./'+'study_'+str(modtype)+str(ID)+'.pkl', 'wb'))
with open('/home/chidesiv/Desktop/Scripts_py/revised_version_scripts/PTWSEQ/20pct_PT/'+'study_'+str(modtype)+str(ID)+str(wavelet)+'PT.pkl','rb') as f:
...
def gwlmodel(init, params):
        with tf.device('/gpu:0'):
            ...
            callbacks = [EarlyStopping(monitor = 'val_loss', mode ='min', verbose = 1, patience=50,restore_best_weights = True), tf.keras.callbacks.ModelCheckpoint(filepath='best_model'+str(modtype)+str(init)+str(ID)+str(wavelet)+'.h5', monitor='val_loss', save_best_only = True, mode = 'min')]
...
for init in range(initm):
    ...
    # globals()["model"+str(init)]=tf.keras.models.load_model('best_model'+str(modtype)+str(init)+str(ID)+str(wavelet)+'.h5')
...
Middle_Target_Index = y_train.max()*0.75 #- y_train.min()/2
# #text
...
# pyplot.text(Middle_Test_Index, Middle_Target_Index,'RMSE =' + str("%.2f" % scores_va.RMSE_va[0]) + '\n MAE=' + str("%.2f" % scores_va.MAE_va[0])+ '\n R2=' + str("%.2f" % scores_va.R2_va[0]),fontsize = 15,bbox=dict(facecolor='none', edgecolor='black', boxstyle='round,pad=1'))
# pyplot.plot( indices_train[f+seq_length-1:],sim_train, 'r', label ="simulated  median train", linewidth = 0.9)
pyplot.title(str(modtype)+' '+ "PT"+' - '+" WT:"+str(wavelet)+ ' Test set RMSE :' + str("%.2f" % scores_va.RMSE_va[0]), size=24,fontweight = 'bold')
...
# pyplot.legend(ncol=2,fontsize=12,loc='upper left')

# pyplot.title(str(modtype)+" Test PT:"+str(ID)+" WT:"+str(wavelet), size=17,fontweight = 'bold')
# pyplot.ylabel('GWL [m asl]', size=15)
# pyplot.xlabel('Date',size=36)
...
pyplot.savefig('./'+'Well_ID'+str(ID)+str(modtype)+str(initm)+str(wavelet)+'PTtestnogrid.png', dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
...
# pd.DataFrame(mae_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PTmae.csv')    
# pd.DataFrame(mse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PTmse.csv')   
# pd.DataFrame(rmse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PTrmse.csv')   
# pd.DataFrame(r2_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+str(wavelet)+'PTr2.csv') 
...
# shap.summary_plot(s, X,feature_names = X_train.columns, show=False)

# plt.savefig('./'+'SHAP'++str(ID)+str(modtype)+str(initm)+'pt.png', dpi=300, bbox_inches = 'tight', pad_inches = 0.1)
# import matplotlib.pyplot as plt
...
#     globals()["e"+str(init)] = shap.DeepExplainer((globals()["model"+str(init)]), X_train_l)
#     globals()["shap_values"+str(init)] = globals()["e"+str(init)].shap_values(X_valid_l)
...
# shap.summary_plot(s, X,plot_type = 'bar',feature_names =  ['P1','P2','P3','P4','P5','T1','T2','T3','T4','T5'], show=False,color_bar=False)              
# plt.colorbar()
# plt.savefig('./'+'SHAP'+str(ID)+str(modtype)+str(initm)+str(wavelet)+'pt.png', dpi=300, bbox_inches = 'tight', pad_inches = 0.1)    