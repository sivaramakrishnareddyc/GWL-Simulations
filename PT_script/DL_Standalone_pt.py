...
with open('/home/chidesiv/Desktop/Scripts_py/revised_version_scripts/PT/20PCT_SA/'+'study_'+str(modtype)+str(ID)+'.pkl','rb') as f:
...
pyplot.title(str(modtype)+' '+ "PT"+' - '+ 'Test set RMSE :' + str("%.2f" % scores_va.RMSE_va[0]), size=24,fontweight = 'bold')
...
pyplot.savefig('./'+'Well_ID'+str(ID)+str(modtype)+str(initm)+'PT_test.png', dpi=600, bbox_inches = 'tight', pad_inches = 0.1)
...
# pd.DataFrame(mae_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+'PTmae.csv')    
# pd.DataFrame(mse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+'PTmse.csv')   
# pd.DataFrame(rmse_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+'PTrmse.csv')   
# pd.DataFrame(r2_init).to_csv('./'+'Well_ID'+str(ID)+str(modtype)+'PTr2.csv') 
